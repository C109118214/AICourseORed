...
with open("read.txt","r") as file:
    ...
    df = pd.read_csv("read.txt", header = None, sep =  " ")
    
    avg_height = df[1].mean()
    avg_weight = df[2].mean()
    max_height = df[1].max()
    max_height_name = df[df[1] == max_height].iloc[0, 0]

    max_weight = df[2].max()
    max_weight_name = df[df[2] == max_weight].iloc[0, 0]
